...
class ManageGeminiApiKeys extends Command
{
    protected $signature = 'gemini:keys
                          {action? : Action to perform (list, add, remove, reset)}
                          {--key= : API key to add or remove}
                          {--name= : Name for the API key}
                          {--model= : Model type for the API key}';

    protected $description = 'Manage Gemini API keys';
    ...
    {
        $keys = GeminiApiKey::all();
        $this->table(
            ['Name', 'Key (partial)', 'Model', 'Request Count', 'Last Used'],
    ...
    protected function removeKey()
    {
        ...
        $deleted = GeminiApiKey::where('key', $key)->delete();
        if ($deleted) {

...
class DeveloperApiController extends Controller
{
    ...
    public function index(Request $request): Response
    {
        ...
        if (!empty($filters['user_query'])) {
            ...
            $keysQuery->whereHas('user', fn ($b) => $b->where('email', 'like', $search)->orWhere('name', 'like', $search));
        }
        ...
        if (!empty($filters['user_query'])) {
            $search = '%' . trim((string) $filters['user_query']) . '%';
            $walletsQuery->whereHas('user', fn ($b) => $b->where('email', 'like', $search)->orWhere('name', 'like', $search));
        }
        ...
        return Inertia::render('Admin/DeveloperApi/Index', [
            ...
            'keys' => $keysQuery->paginate(12)->withQueryString()->through(fn (DeveloperApiKey $key) => [
                ...
                'user' => $key->user ? ['id' => $key->user->id, 'name' => $key->user->name, 'email' => $key->user->email] : null,
            ]),
            'usage' => $usageQuery->latest()->paginate(20)->withQueryString()->through(fn (DeveloperUsageRecord $usage) => [
                'id' => $usage->id,
                ...
                'user' => $usage->user ? ['id' => $usage->user->id, 'name' => $usage->user->name, 'email' => $usage->user->email] : null,
                'api_key' => $usage->apiKey ? ['id' => $usage->apiKey->id, 'name' => $usage->apiKey->name, 'key_prefix' => $usage->apiKey->key_prefix] : null,
                'model' => $usage->model ? ['id' => $usage->model->id, 'public_id' => $usage->model->public_id, 'name' => $usage->model->name] : null,
            ]),
            ...
            'ledger' => $ledgerQuery->latest()->paginate(15)->withQueryString()->through(fn ($entry) => [
                'id' => $entry->id,
                'type' => $entry->type,
                ...
                'api_key' => $entry->apiKey ? ['id' => $entry->apiKey->id, 'name' => $entry->apiKey->name, 'key_prefix' => $entry->apiKey->key_prefix] : null,
            ]),
            'stats' => [
            ...
            'breakdowns' => [
                ...
                'recent_payments' => (clone $ledgerQuery)
                    ...
                    ->map(fn ($e) => [
                        'id' => $e->id,
                        'amount_usd' => round((float) $e->amount_usd, 6),
    ...
    public function storeModel(Request $request): RedirectResponse
    {
        ...
        ApiModel::create(array_merge($validated, ['model_type' => 'text']));

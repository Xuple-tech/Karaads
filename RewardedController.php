...
class RewardedController extends Controller
{
    ...
    private function existingWithdrawalResponse(string $userId, string $idempotencyKey, string $requestFingerprint): ?JsonResponse
    {
        ...
        $firstMeta = is_array($items->first()->meta) ? $items->first()->meta : [];
        $storedFingerprint = (string) ($firstMeta['withdrawal_request_fingerprint'] ?? '');
        ...
        $wallet = UserWallet::query()->where('user_id', $userId)->first();
        $reference = (string) (
            $firstMeta['withdrawal_reference']
            ?? $firstMeta['failed_withdrawal_reference']
            ?? $this->buildWithdrawalReference($idempotencyKey)
        );
        $transfer = is_array($firstMeta['withdrawal_response'] ?? null) ? $firstMeta['withdrawal_response'] : null;

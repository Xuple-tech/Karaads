...
const Edit = ({ stat, agents }) => {
    const { data, setData, put, errors, processing } = useForm({
        agent_id: stat.agent_id,
        date: stat.date,
        conversations_count: stat.conversations_count,
        messages_count: stat.messages_count,
        users_count: stat.users_count,
        avg_response_time: stat.avg_response_time,
        satisfaction_score: stat.satisfaction_score,
        common_questions: stat.common_questions || [],
        peak_hours: stat.peak_hours || [],
        knowledge_base_hits: stat.knowledge_base_hits,
        tool_usage: stat.tool_usage || {},
    });
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        put(route('admin.agent-usage-stats.update', stat.id));
    };
    ...
    return (
        <AdminLayout>
            <Head title={`Edit Statistics - ${stat.date}`} />

            <div className="space-y-6">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight">Edit Usage Statistics</h1>
                    <p className="text-muted-foreground">
                        Update usage statistics for {stat.agent?.name} on {stat.date}
                    </p>
                </div>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div className="flex justify-end space-x-4">
                                ...
                                <Button type="submit" disabled={processing}>
                                    {processing ? 'Updating...' : 'Update Statistics'}
                                </Button>
                            </div>
                        </form>
                    </CardContent>

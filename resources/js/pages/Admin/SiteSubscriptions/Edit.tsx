...
const Edit = ({ subscription, sites, plans, users, statuses, billingCycles, currencies }) => {
    ...
    const [priceOverride, setPriceOverride] = useState(true);

    const handleSubmit = (e) => {
        e.preventDefault();
        put(route('admin.site-subscriptions.update', subscription.id));
    };
    ...
    return (
        <AdminLayout>
            <Head title={`Edit Subscription - ${subscription.site?.name}`} />

            <div className="space-y-6">
                <div>
                    <h1 className="text-3xl font-bold tracking-tight">Edit Subscription</h1>
                    <p className="text-muted-foreground">
                        Update subscription details for {subscription.site?.name}
                    </p>
                </div>

...
class NewPostFromConnectionNotification extends Notification
{
    ...
    public function toMail(object $notifiable): MailMessage
    {
        ...
        $message = (new MailMessage)
            ->mailer(config('mail.notification_mailer', 'vps'))
            ->from(
                config('mail.notification_from.address', config('mail.from.address')),
                config('mail.notification_from.name', config('mail.from.name'))
            )

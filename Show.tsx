...
interface TransactionItem {
    id: string;
    type: 'earning' | 'rewarded_withdrawal' | 'withdrawal_request';
    label: string;
    reference: string;
    amount: number;
    status: string;
...
export default function Show({
    ...
    const earningsTotal = earningsHistory.reduce((sum, earning) => sum + Number(earning.amount || 0), 0);
    const verificationCategory = verificationRequest?.category?.replace(/_/g, ' ') || 'unknown';
            ...
            <AdminLayout>
                <AdminPage>
                    ...
                    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
                        ...
                        <AdminMetricCard label="Earnings" value={formatCurrency(earningsTotal, walletCurrency)} hint="All earnings recorded for this user" icon={DollarSign} />
                    </div>

                    <AdminSection title="Transactions" description="Latest money movement for this user.">
                        <AdminPanel title="Recent credits and debits" description="Shown immediately when an admin opens the user record.">
                                            ...
                                            transactions.map((transaction) => {
                                                ...
                                                return (
                                                    <TableRow key={transaction.id}>
                                                        ...
                                                        <TableCell className="font-medium">{transaction.reference || 'N/A'}</TableCell>
                                                        <TableCell className={isCredit ? 'text-emerald-600' : 'text-rose-600'}>
                                                            {amountPrefix}
                                                        ...
                                                        <TableCell>
                                                            {transaction.occurred_at ? new Date(transaction.occurred_at).toLocaleString() : 'N/A'}
                                                        </TableCell>
                                                    </TableRow>
                                                );
                                            })
                    ...
                    <AdminSection title="Earnings history" description="Complete earnings entries for this user.">
                        <AdminPanel title="Full earnings ledger" description="This shows every earning record currently stored for the user.">
                            <div className="overflow-x-auto">
                                <Table>
                                    <TableHeader>
                                        <TableRow>
                                    ...
                                    <TableBody>
                                        {earningsHistory.length > 0 ? (
                                            earningsHistory.map((earning) => {
                                                const occurredAt = earning.paid_at || earning.created_at;
                                                ...
                                                return (
                                                    <TableRow key={earning.id}>
                                                        ...
                                                        <TableCell className="font-medium">{earning.reference || 'N/A'}</TableCell>
                                                        <TableCell className="text-emerald-600">
                                                            +{formatCurrency(earning.amount, walletCurrency)}
                                                        </TableCell>
                                                        <TableCell>
                                                            <Badge variant="outline" className="capitalize">
                                                        ...
                                                        <TableCell className="max-w-[320px] truncate text-sm text-muted-foreground">
                                                            {earning.description || (earning.post_id ? `Post: ${earning.post_id}` : 'No description')}
                                                        </TableCell>
                                                        <TableCell>{occurredAt ? new Date(occurredAt).toLocaleString() : 'N/A'}</TableCell>
                                                    </TableRow>
                                                );
                                            })
                                        ) : (
                    ...
                    <AdminSection title="Platform withdrawals" description="Latest withdrawals across all users.">
                        <AdminPanel title="All user withdrawals" description="This list shows the newest manual and rewarded withdrawals from the platform.">
                            <div className="overflow-x-auto">
                                <Table>
                                    <TableHeader>
                                        <TableRow>
                                        ...
                                        {platformWithdrawals.length > 0 ? (
                                            platformWithdrawals.map((withdrawal) => (
                                                <TableRow key={withdrawal.id}>
                                                    <TableCell>
                                                        <Badge variant={withdrawal.type === 'rewarded' ? 'default' : 'secondary'} className="capitalize">
                                                            {withdrawal.label}
                                                        </Badge>
                                                    </TableCell>
                                                    <TableCell>
                                                    ...
                                                    <TableCell className="max-w-[220px] truncate font-mono text-xs">
                                                        ...
                                                        {withdrawal.note ? <div className="mt-1 truncate text-[11px] font-sans text-muted-foreground">{withdrawal.note}</div> : null}
                                                    </TableCell>
                                                    <TableCell>
                                                        {withdrawal.occurred_at ? new Date(withdrawal.occurred_at).toLocaleString() : 'N/A'}
                                                    </TableCell>
                                    ...
                                    {verificationRequest ? (
                                        <div className="space-y-4">
                                            <div className="flex flex-wrap items-center gap-2">
                                                ...
                                                <Badge variant="secondary" className="capitalize">{verificationCategory}</Badge>
                                            </div>
                                            ...
                                            withdrawals.map((withdrawal) => (
                                                <TableRow key={withdrawal.reference}>
                                                    ...
                                                    <TableCell className="text-sm text-muted-foreground">
                                                        <div>Bank: {withdrawal.bank_name || withdrawal.bank_code || 'N/A'}</div>
                                                        <div>Account: {withdrawal.account_number || 'N/A'}</div>
                                                        <div>Name: {withdrawal.account_name || 'N/A'}</div>
                                                        <div>Provider ref: {withdrawal.provider_reference || 'Pending provider ref'}</div>
